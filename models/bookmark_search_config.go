...
var (
	...
	bookmarkSearchConfigGeneratedColumns      = []string{}
)
...
func BookmarkSearchConfigs(mods ...qm.QueryMod) bookmarkSearchConfigQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"bookmark_search_config\".*"})
	}

	return bookmarkSearchConfigQuery{q}
}
...
// Exists checks if the BookmarkSearchConfig row exists.
func (o *BookmarkSearchConfig) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return BookmarkSearchConfigExists(ctx, exec, o.K)
}
